...
const Post = ({ id, post, postPage }) => {
    ...
    return (
        <div className="p-3 flex cursor-pointer border-b border-gray-700" onClick={() => router.push(`/${id}`)}>
            {!postPage && (<img className="h-11 w-11 rounded-full mr-4" src={post?.userImg} alt="" />)}

            <div className="flex flex-col space-y-2 w-full">
                <div className={`flex ${!postPage && "justify-between"}`}>
                    {postPage && (<img className="h-11 w-11 rounded-full mr-4" src={post?.userImg} alt="Profile Pic" />)}
                ...
                <img className="rounded-2xl max-h-[700px]" src={post?.image} alt="" />

                {/* Icons in post section */}
